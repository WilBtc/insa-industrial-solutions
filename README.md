# INSA Ingeniería - Industrial Automation & AI Solutions

<div align="center">

### 🏭 Digital Transformation for Industry 4.0

**Industrial IoT | Smart Analytics | AI-Powered Automation | Cybersecurity**

*Delivering cutting-edge automation solutions for Oil & Gas, Mining, Manufacturing, and Critical Infrastructure*

[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)
[![Industry 4.0](https://img.shields.io/badge/Industry-4.0-orange.svg)]()
[![AI Powered](https://img.shields.io/badge/AI-Powered-green.svg)]()

**English** | [Español](./README.es.md)

</div>

---

## 🎯 About INSA Ingeniería

**INSA Ingeniería SAS** (Colombia/Ecuador) and **INSA Automation Corp** (USA) specialize in intelligent industrial automation, combining decades of engineering expertise with cutting-edge AI and IoT technologies. With **35+ employees** including **~18 engineers and technicians**, we deliver enterprise-grade solutions across Latin America and North America.

### Our Track Record
- 🏆 **161.4+ million IoT sensor records** managed in production
- 🏆 **$490K+ active project pipeline** in oil & gas sector
- 🏆 **99.9% uptime** on deployed industrial systems
- 🏆 **2+ years** Argentina market presence (Vaca Muerta expansion)
- 🏆 **50+ major clients** including PAREX Resources, Verano Energy, Gran Tierra Energy

### Our Expertise
- ⚡ **Industrial IoT Platforms** - 109M+ records, 47+ active sensors, real-time control
- 📊 **Smart Reporting & Analytics** - AI-powered dashboards and compliance automation
- 🤖 **AI-Powered Automation** - Predictive maintenance, LSTM forecasting, autonomous agents
- 🔒 **Industrial Cybersecurity** - IEC 62443 compliance, DefectDojo, Wazuh SIEM
- 🛢️ **Oil & Gas Solutions** - ESP/VFD optimization with 83% run-life improvement
- 🏭 **Manufacturing & Glass** - Vidrio Andino S.A.S deployment (7 devices, 75 sensors)

---

## 🚀 Our Solutions Portfolio

### 1. 🌐 INSA IoT Platform
**Real-time Industrial Monitoring & Control**

Enterprise-grade IoT platform for industrial equipment monitoring, data acquisition, and remote control.

**Key Features:**
- Real-time telemetry from PLCs, SCADA, and industrial sensors
- Multi-protocol support (Modbus, OPC-UA, MQTT, LoRaWAN)
- Advanced data visualization and dashboards
- Predictive maintenance algorithms
- Mobile and web interfaces
- Edge computing capabilities

**Industries:**
- Oil & Gas (ESP, VFD, wellhead monitoring)
- Mining (equipment tracking, environmental monitoring)
- Manufacturing (production line monitoring)
- Utilities (SCADA integration)

📁 [View IoT Platform →](./iot-platform)

---

### 2. 📈 ANH Smart Reporter
**Automated Regulatory Compliance & Reporting**

AI-powered reporting system for the Colombian Oil & Gas sector, automating ANH (Agencia Nacional de Hidrocarburos) compliance.

**Key Features:**
- Automated data collection from field operations
- Intelligent report generation
- Regulatory compliance validation
- Real-time production analytics
- Historical trend analysis
- Multi-field aggregation

**Benefits:**
- 95% reduction in manual reporting time
- 100% compliance with ANH regulations
- Real-time production insights
- Automated anomaly detection

📁 [View Smart Reporter →](./anh-smart-reporter)

---

### 3. 🤖 AI-Powered Autonomous Systems
**Intelligent Automation for Industrial Operations**

Advanced AI agents and autonomous systems for industrial process optimization and cybersecurity.

#### AI SecOps Platform
- Automated security monitoring and threat detection
- Compliance tracking (IEC 62443, NIST, ISO 27001)
- Vulnerability scanning and remediation
- Security incident response automation

#### Autonomous Industrial Agents
- Predictive maintenance agents
- Process optimization algorithms
- Anomaly detection and alerting
- Self-healing system capabilities

📁 [View AI Solutions →](./ai-solutions)

---

### 4. 🛢️ ESP/VFD Optimization Suite
**Artificial Lift Optimization for Oil & Gas**

AI-driven optimization for Electric Submersible Pumps (ESP) and Variable Frequency Drives (VFD).

**Capabilities:**
- Real-time performance monitoring
- Predictive failure analysis
- Automated parameter optimization
- Energy consumption reduction
- Production maximization
- Downtime prevention

**Results:**
- Up to 30% energy savings
- 40% reduction in unexpected failures
- 25% increase in production efficiency

📁 [View ESP/VFD Suite →](./esp-vfd-optimization)

---

### 5. 📊 Industrial Data Analytics
**Advanced Analytics & Business Intelligence**

Enterprise data analytics platform with AI-powered insights for industrial operations.

**Features:**
- Real-time production dashboards
- Predictive analytics
- Equipment performance tracking
- Cost optimization analysis
- Environmental compliance monitoring
- Custom KPI tracking

**Integrations:**
- Grafana & Prometheus monitoring
- ThingsBoard IoT platform
- ERP/CRM systems
- SCADA/DCS systems

📁 [View Analytics Platform →](./data-analytics)

---

## 🤝 Major Clients & Projects

### Oil & Gas Sector
**Active Clients in Colombia:**
- 🛢️ **PAREX Resources** - SC8038-3 Filtros Canasta ($8K USD)
- 🛢️ **Verano Energy (Switzerland) AG** - 2 active deals ($43.5K USD total)
  - SC8048 TIE IN ODCA ($29K USD) - Prepayment stage
  - SC8062 TIE IN ODCA Opción 3 ($14.5K USD)
- 🛢️ **Gran Tierra Energy Colombia** - SC7493 Ecuador project ($49.8K USD)
- 🛢️ **CFRO** - Won SC6785-2 Tanque almacenamiento ($88.2K USD - largest contract)
- 🛢️ **New Granada Energy, SAR Energy, Rayogas, TURGAS**
- 📊 **Total Active Pipeline**: $490K+ USD (1.96B COP)

**Argentina Market Entry:**
- 🎯 **Target**: Vaca Muerta Shale Formation (Neuquén Basin)
- 🤝 **Partner**: BLS LLC (Black Label Services - Colorado, USA)
- 🏢 **Prospects**: GeoPark, YPF (state oil company)
- 📅 **Status**: 2 years market research, trade show presence

### Manufacturing & Industrial
- 🏭 **Vidrio Andino S.A.S** - Glass Manufacturing IoT Platform
  - **161.4 million** historical sensor records (Aug-Oct 2025)
  - **109+ million** active records in production
  - **7 IoT devices** deployed across 6 industrial areas
  - **75 sensor keys** monitoring temperature, humidity, production
  - **Industrial Areas**: Empaque, Laminado, Muelles, Naves A-F
  - **Platform**: ThingsBoard Pro + Custom INSA IoT Platform v2.0
  - **Uptime**: 99.9% target with real-time monitoring

### International Presence
- 🇨🇴 **Bogotá, Colombia** (Headquarters - 35 employees)
- 🇪🇨 **Ecuador** (Operations)
- 🇺🇸 **Texas, USA** (Midland - Oil & Gas hub)
- 🇦🇷 **Argentina** (Market expansion - Vaca Muerta)

---

## 🏗️ Technology Stack

### Core Technologies
- **IoT & Edge:** ThingsBoard, MQTT, Modbus, OPC-UA, LoRaWAN
- **AI/ML:** Python, TensorFlow, PyTorch, Scikit-learn
- **Data:** PostgreSQL, TimescaleDB, InfluxDB, Redis
- **Cloud:** Azure, AWS, Docker, Kubernetes
- **Monitoring:** Grafana, Prometheus, Wazuh, DefectDojo
- **Integration:** n8n, Apache Airflow, RabbitMQ

### Development Practices
- ✅ CI/CD automation
- ✅ Security-first architecture
- ✅ ISO 27001 & IEC 62443 compliance
- ✅ Comprehensive testing
- ✅ Documentation-driven development

---

## 📋 Industry Solutions

### 🛢️ Oil & Gas
- ESP/VFD monitoring and optimization
- Pipeline monitoring and leak detection
- Tank level monitoring
- Production optimization
- ANH regulatory compliance
- Remote wellhead monitoring

### ⛏️ Mining
- Equipment tracking and monitoring
- Environmental compliance
- Safety monitoring systems
- Production analytics
- Energy management

### 🏭 Manufacturing
- Production line monitoring
- Quality control automation
- Predictive maintenance
- OEE optimization
- Supply chain integration

### 🏗️ Infrastructure
- Building automation
- Energy management
- Security systems integration
- Environmental monitoring

---

## 🌟 Why Choose INSA?

### ✨ **Proven Expertise**
20+ years of industrial automation experience combined with cutting-edge AI capabilities

### 🔧 **Custom Solutions**
Tailored systems designed for your specific industrial requirements

### 🚀 **Rapid Deployment**
Fast implementation with minimal disruption to operations

### 📞 **24/7 Support**
Round-the-clock technical support and monitoring

### 💰 **ROI Focused**
Solutions designed to deliver measurable business value

### 🔒 **Security First**
Industrial cybersecurity built into every solution

---

## 📞 Contact & Support

**INSA Ingeniería SAS / INSA Automation Corp**

### Colombia/Ecuador Operations
- 👤 **Juan Carlos Casas** - CEO, Technical Solutions
- 📧 Email: j.casas@insaing.com
- 👥 Team: 35 employees (~18 engineers/technicians)
- 📍 Location: Bogotá, Colombia (Headquarters) | Ecuador (Operations)

### USA Operations
- 👤 **Wil Aroca** - US Operations Manager, AI/ML Lead
- 📧 Email: w.aroca@insaing.com
- 📍 Location: Texas (Midland - Oil & Gas market)

### Company Information
- 🌐 Domain: insaing.com
- 💼 CRM: insa.bitrix24.es
- 🔗 GitHub: [@WilBtc](https://github.com/WilBtc)

### Get Started
Interested in transforming your industrial operations? Contact us for:
- ✅ Solution demonstrations (Live IoT platform with 109M+ records)
- ✅ Technical consultations (AI/ML, ESP optimization, IEC 62443)
- ✅ Custom project quotes (Oil & Gas, Manufacturing, Mining)
- ✅ Proof of concept deployments (30-90 day pilots)
- ✅ Partnership opportunities (BLS partnership model available)

---

## 📄 License

This repository contains documentation and public-facing information about INSA's industrial solutions.

Individual solution components may have different licensing terms. Please contact us for licensing information.

---

## 🔗 Related Resources

- [Technical Documentation](./docs)
- [Case Studies](./case-studies)
- [API Documentation](./api-docs)
- [Integration Guides](./integrations)

---

<div align="center">

**Transforming Industry Through Intelligent Automation**

🏭 Industrial IoT • 🤖 AI & Machine Learning • 📊 Data Analytics • 🔒 Cybersecurity

*Building the future of industrial automation, one solution at a time.*

</div>
